...
foreach ($fixtureName in $validFixtures) {
    ...
    if ($fixtureName -eq "temporary_receiver_method.k" -and $stage1Exit -ne 42) {
        Write-Error "temporary receiver method fixture did not return 42"
    }
    ...
    if ($fixtureName -eq "range_membership.k" -and $stage1Exit -ne 42) {
        Write-Error "range membership fixture did not return 42"
    }
    ...
    if ($fixtureName -eq "when_control.k" -and $stage1Exit -ne 42) {
        Write-Error "when control fixture did not return 42"
    }
