...
Compiler bootstrap có `StringBuilder` move-only riêng cho việc dựng UTF-8 tuần tự. API chính gồm `StringBuilder.new()`, `append(string)` và `toString()`; `add(u8)` và `appendBytes(u8*, u64)` chỉ dành cho encode hoặc dữ liệu thô. `append(string)` đọc qua borrowed `text.bytes`, không copy hay nhận ownership của string đầu vào. `toString()` tạo một owned `string` deep-copy nên builder vẫn có thể được dùng tiếp; cả builder và string kết quả đều được auto-drop. LLVM text emitter, command line và diagnostic text của compiler K đã dùng builder này; `ByteBuffer` chỉ còn ở loader cho path/source byte storage. Đây là builder dành cho compiler, không làm cho bản thân `string` trở thành mutable byte buffer.
